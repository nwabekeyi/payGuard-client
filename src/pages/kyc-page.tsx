...
import { IconShield, IconCheck, IconBuilding } from "../components/common/icons";
...
export default function KycPage() {
  ...
  const hasPreExistingBank = user?.hasBankAccount || false;
  const [isBankSectionSkipped, setIsBankSectionSkipped] = useState(hasPreExistingBank);
  ...
  useEffect(() => {
    bankService.getAllBanks()
      ...
      .catch(err => setBanksError(err instanceof Error ? err.message : "Failed to load banks"))
      .finally(() => setBanksLoading(false));
  }, []);

  useEffect(() => {
    if (hasPreExistingBank && user?.bankAccountNumber && user?.bankCode) {
      setBankAccountNumber(user.bankAccountNumber);
      setBankCode(user.bankCode);
    }
  }, [hasPreExistingBank, user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await identityService.verifyIdentity({ 
        kycType, 
        kycId,
        ...( !hasPreExistingBank ? { bankAccountNumber, bankCode } : {} )
      });
   ...
   return (
     <div className="max-w-2xl mx-auto py-8 animate-fade-in">
       ...
       <form onSubmit={handleSubmit} className="bg-white border border-gray-100 shadow-xl shadow-gray-200/40 rounded-[2rem] p-8 md:p-10 space-y-8">
          ...
          {/* Bank Section - only show if user doesn't already have a bank linked */}
          {!hasPreExistingBank && (
          <div>
                   ...
                   <div className="space-y-1.5">
                      ...
                      <div className="relative">
                          <select
                              ...
                              className="w-full px-4 py-3 bg-gray-50/50 border border-gray-200/80 text-gray-900 rounded-xl font-mono focus:outline-none focus:ring-2 focus:ring-primary-500/30 transition-all font-medium appearance-none"
          ...
          <div className="pt-4 border-t border-gray-100">
                <Button
                  type="submit"
                  disabled={loading || kycId.length !== 11 || (!hasPreExistingBank && (bankAccountNumber.length !== 10 || !bankCode)) || banksLoading}
